...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        #add
        # base_feature.requires_grad_(True)
        rep_variable = Variable(base_feature.data.clone(), requires_grad=True)
        # a = base_feature.grad
        # print(a)
        list_rep = False
        ...
        grads = {}
        losses_rpn = losses[0] + losses[1]
        losses_roi = losses[2] + losses[3]
        losses_rpn.backward()
        grads[0] = []
        grads[0].append(Variable(rep_variable.grad.data.clone(), requires_grad=False))
        rep_variable.grad.data.zero_()
        losses_roi.backward()
        grads[1] = []
        grads[1].append(Variable(rep_variable.grad.data.clone(), requires_grad=False))
        rep_variable.grad.data.zero_()
    ...
    def train_step(self, imgs, bboxes, labels, scale, fp16=False, scaler=None):
        ...
        if not fp16:
            ...
            losses, grads = self.forward(imgs, bboxes, labels, scale)
            # grads = {}
            # grads[0] = []
            # grads[1] = []
            #
            ...
            # losses_roi = losses[2] + losses[3]
            # losses_rpn.backward()
            # for param in self.model_train.parameters():
            ...
            # losses[-1].backward()
            # loss_data = []
            # loss_data.append(losses_rpn.item())
            # loss_data.append(losses_roi.item())
            ...
            scale_ = {}
            sol, min_norm = MinNormSolver.find_min_norm_element([grads[t] for t in [0, 1]])
            scale_[0] = float(sol[0])
            scale_[1] = float(sol[1])
            print('scale_rpn:', scale_[0], '  ', 'scale_roi:', scale_[1])
            ...
            losses_rpn = losses[0] + losses[1]
            losses_roi = losses[2] + losses[3]
            loss = scale_[0]*losses_rpn + scale_[1]*losses_roi
            loss.backward()
            self.optimizer.step()
        else:
