...
def fit_one_epoch(model, train_util, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir):
    ...
    with tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            rpn_loss = rpn_loc.item() + rpn_cls.item()
            roi_loss = roi_loc.item() + roi_cls.item()
    ...
    loss_history.append_loss(epoch + 1, total_loss / epoch_step, val_loss / epoch_step_val, rpn_loss, roi_loss)
